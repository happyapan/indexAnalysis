...
base = BaseStock("base")
mork_ball = MorkBollBackDownLine(20, -3, 9, 3, 5)
...
with open(IndexConstants.date_file_path + "20_3_9_3_5_60.txt", 'at') as file:
...
savedStdout = sys.stdout  # 保存标准输出流
with open(IndexConstants.date_file_path + "20_3_9_3_5_30.txt", 'at') as file:
    sys.stdout = file  # 标准输出重定向至文件
    for one in stock:
        mork_ball.mork_bollBackDownLine(one, stock[one], statistics_data)
sys.stdout = savedStdout  # 恢复标准输出流
...
savedStdout = sys.stdout  # 保存标准输出流
with open(IndexConstants.date_file_path + "20_3_9_3_5_00.txt", 'at') as file:
    sys.stdout = file  # 标准输出重定向至文件
    for one in stock:
        mork_ball.mork_bollBackDownLine(one, stock[one], statistics_data)
sys.stdout = savedStdout  # 恢复标准输出流