...
class MorkB5N(object):
            ...
            if catch_stock is not None:
                ...
                if len(after_datas) >= 1:
                    ...
                    if catch_day_macd is not None \
                        ...
                        self.cal_print_avg_line(catch_stock, avg_result, after_datas, 0, test_trade, macd_result)
                        for cal_day in range(2, self._calDay+1):
                            if len(after_datas) >= cal_day:
                                self.cal_print_avg_line(catch_stock, avg_result, after_datas, cal_day-1, test_trade, macd_result)
    ...
    def cal_print_avg_line(self, catch_stock, avg_result, after_datas, day_count, test_trade, macd_result):
