...
with open(IndexConstants.date_file_path + "MorkB5N20_3_9_1_1_Z_60.txt", 'at') as file:
...
savedStdout = sys.stdout  # 保存标准输出流
with open(IndexConstants.date_file_path + "MorkB5N20_3_9_1_1_Z_30.txt", 'at') as file:
    sys.stdout = file  # 标准输出重定向至文件
    for one in stock:
        mork_ball.mork_b5m(one, stock[one], statistics_data)
sys.stdout = savedStdout  # 恢复标准输出流
...
savedStdout = sys.stdout  # 保存标准输出流
with open(IndexConstants.date_file_path + "MorkB5N20_3_9_1_1_Z_00.txt", 'at') as file:
    sys.stdout = file  # 标准输出重定向至文件
    for one in stock:
        mork_ball.mork_b5m(one, stock[one], statistics_data)
sys.stdout = savedStdout  # 恢复标准输出流