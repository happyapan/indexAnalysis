...
# 猪肉
pigDate = PigData("猪肉")
p_file('./dataFiles/qihuo' + today_date + '.txt', analysis.change_rate_day(pigDate.get_data(), cycle_index),
       pigDate.name)

# 鸡蛋
p_file('./dataFiles/qihuo' + today_date + '.txt', analysis.change_rate_day(
...
for key in all_qi_huo:
    p_file('./dataFiles/qihuo' + today_date + '.txt', analysis.change_rate_day(mofComData.get_data(key), cycle_index),
           all_qi_huo[key])
